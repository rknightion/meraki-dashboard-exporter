...
class TestMXCollector:
    ...
    async def test_collect_uplink_statuses_basic(
        ...
        call_args_list = mock_parent._set_metric.call_args_list
        _, kwargs0 = call_args_list[0]
        labels0 = kwargs0.get("labels", call_args_list[0][0][1])
        assert labels0["serial"] == "Q2AB-1234-5678"
        assert labels0["name"] == "Office MX"
        assert labels0["interface"] == "wan1"
        assert labels0["status"] == "active"
        assert labels0["network_name"] == "Office Network"

        # Verify the second call (wan2 not connected)
        labels1 = call_args_list[1][0][1]
        assert labels1["interface"] == "wan2"
        assert labels1["status"] == "not connected"
    ...
    async def test_collect_uplink_statuses_unknown_serial(
        ...
        labels = mock_parent._set_metric.call_args_list[0][0][1]
    ...
    async def test_collect_uplink_statuses_uses_device_type_from_model(
        self,
        mx_collector: MXCollector,
        mock_api: MagicMock,
        mock_parent: MagicMock,
