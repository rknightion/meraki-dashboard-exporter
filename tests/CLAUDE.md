...
<file_map>
## TEST ORGANIZATION
- `conftest.py` - Shared fixtures: `fast_test_settings` (disables retry/smoothing), `clean_prometheus_registry` (autouse, wipes the global `REGISTRY` before/after every test), `isolated_registry`, `force_debug_log_capture`.
...
class TestMyCollector(BaseCollectorTest):
    ...
    # No update_tier — the attribute was removed with the tier system (#631);
    # cadence is derived from the collector's declared endpoint groups instead.
