...
| `meraki_client_info` | gauge | `org_id`, `network_id`, `client_id`, `mac`, `description`, `hostname`, `ip`, `ip6`, `ssid` | Client information join metric (client_id -> mac/description/hostname/ip/ip6/ssid); value is always 1. The superseded series is removed as soon as a client's hostname/description/IP/SSID changes, so exactly one series per client is exposed. | Requires MERAKI_EXPORTER_CLIENTS__ENABLED=true |
