...
<file_map>
- `generate-docs.sh` - orchestrator; runs all 4 generators in a fixed order (config, metrics, collectors, endpoints) via `uv run python` (falls back to plain `python3` if `uv` isn't on PATH). This is what `make docgen` calls.
