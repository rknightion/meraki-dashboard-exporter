...
- `MERAKI_EXPORTER_SERVER__API_TOKEN`: Bearer token that enables the two state-changing control POSTs (`/api/collectors/trigger`, `/api/clients/clear-dns-cache`). When unset (default), those POSTs fail closed with HTTP 401 and their UI controls are disabled. When configured, the controls are API-only and hidden from the browser. The sensitive GET UIs remain open by default; bind the exporter to a trusted network or disable them. See [security.md](docs/security.md#endpoint-authentication).
