<system_context>
Supporting services for the Meraki Dashboard Exporter - shared inventory caching, client data storage, DNS resolution, and the `/status` health-dashboard aggregator. `inventory.py` is the single enforcement point for filtered network-list reads and the normal shared path for device and availability reads (see root `CLAUDE.md` fatal_implications).
</system_context>

<critical_notes>
- **`OrganizationInventory` (`inventory.py`) is the mandatory network-list path and the normal shared device/availability path.** It is constructed once in `collectors/manager.py::CollectorManager.__init__` with a live `NetworkFilter(settings.network_filter)` and shared by all collectors as `self.inventory` / `parent.inventory`. Direct `getOrganizationNetworks` calls are forbidden outside the three documented root-level exceptions. Existing device exceptions are the MT sensor-only `getOrganizationDevices` fetch and inventory-unavailable device-availability fallbacks in `collectors/device.py` and `collectors/organization.py`; do not add another without updating the project contract.
...
- **`unfiltered=True` is a tightly bounded escape hatch.** It exists on `get_networks`, `get_devices`, and `get_device_availabilities` for: (1) internal recursive calls that need the full network set to resolve `allowed_ids` (see `get_devices`/`_maybe_filter_availabilities` falling back to `get_networks(org_id, unfiltered=True)` on a cache miss), and (2) `CollectorManager._validate_network_filter()`, which compares the full and resolved network sets during startup validation. `core/discovery.py::DiscoveryService` obtains its audit inventory through the separate sanctioned direct-SDK bypass. Do not add new `unfiltered=True` call sites in collectors.
